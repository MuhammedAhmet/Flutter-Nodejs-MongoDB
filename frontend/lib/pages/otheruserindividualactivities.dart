...
import 'package:frontend/pages/otheruserdetailpage.dart';
...
class OtherUsrIndividualIndexPage extends StatefulWidget {
  @override
  _OtherUsrIndividualIndexPageState createState() => _OtherUsrIndividualIndexPageState();
}

class _OtherUsrIndividualIndexPageState extends State<OtherUsrIndividualIndexPage> {
  List activities = [];
  List activities2 = [];
  ...
  fetchUser() async {
    ...
    for(int i=0;i<activities.length;i++){
      if(activities[i]['creatorUsername'] == getusername){
  ...
  Widget getCard(item){
    ...
    return InkWell(
        onTap: (){
          ...
          Navigator.push(
              context, new MaterialPageRoute(builder: (context) => OtherUsrDetailPage())
          );
        },
        child: Card(
